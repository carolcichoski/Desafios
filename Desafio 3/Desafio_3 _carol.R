...
## Obs: origem: repositório do curso e página de fonte de dados.
...
remove(path_baseperfil) #remove dados
...
## dados acessados em https://siconfi.tesouro.gov.br/siconfi/pages/public/consulta_finbra/finbra_list.jsf
zipfile <- "finbra_MUNEST_DespesasporFuncao(AnexoI-E).zip"
unzip(zipfile)
file.remove(zipfile)
list.files()

#carregando dados - FINBRA
...
# Resposta: Poucos municipios de RO participa participam de consórcio de saúde.
...
baseperfilsus3 <- baseperfilsus %>%
 

ggplot(baseperfilsus3) +
  ...
  ggtitle("Número de óbitos infantis em relação à partipação em consórcio por município RO") +
...
ggplot(baseunica) +
  ...
  geom_jitter(aes(x=p, y=menor1ano)) +
